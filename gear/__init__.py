...
class Server(BaseClientServer):
    ...
    def __init__(self, port=4730, ssl_key=None, ssl_cert=None, ssl_ca=None,
        ...
        # Register the wake pipe so that we can break if we need to
        # reconfigure connections
        self.poller.register(self.wake_read, self.poller.POLL_IN
                             | self.poller.POLL_ERR
                             | self.poller.POLL_HUP
                             | self.poller.POLL_EDGE)
    ...
    def _processPollEvent(self, conn, event):
        ...
        try:
            ...
            self.log.exception('DisconnectError')
    ...
    def _flushAllConnections(self):
        ...
        connections = self.active_connections[:]
        self.log.debug('Connections: %s', connections)
        for conn in connections:
            self._processPollEvent(conn, self.poller.POLL_IN
                                   | self.poller.POLL_OUT)
    ...
    def _registerConnection(self, conn):
        ...
        self.poller.register(conn.conn.fileno(), self.poller.POLL_IN
                             | self.poller.POLL_OUT
                             | self.poller.POLL_ERR
                             | self.poller.POLL_HUP
                             | self.poller.POLL_EDGE)
